    ...
    if menu_selection == "Search":
        ...
        if not st.session_state.active_search_label:
            ...
            if not results:
                ...
                # Trigger top scroll if search query changed
                if st.session_state.active_search_label != st.session_state.prev_search_label:
                    st.session_state.prev_search_label = st.session_state.active_search_label
                    
                    # Direct JS injection into top-level parent DOM
                    st.markdown(
                        """
                        <img src="x" onerror="
                            const mainContainer = document.querySelector('[data-testid=stMain]') || 
