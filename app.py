...
def render_pagination_bar(
    page_key, current_page, total_pages, total_items, key_suffix="bottom"
):
  """Reusable bottom toolbar for page navigation controls."""
  p_col1, p_col2, p_col3, p_col4, p_col5 = str_lit.columns(
      [1, 1, 3, 1, 1], vertical_alignment="center"
  )

  with p_col1:
    if str_lit.button(
        "⏮️", key=f"{page_key}_first_{key_suffix}", disabled=current_page == 1
    ):
      str_lit.session_state[page_key] = 1
      str_lit.rerun()
  with p_col2:
    if str_lit.button(
        "◀️", key=f"{page_key}_prev_{key_suffix}", disabled=current_page == 1
    ):
      str_lit.session_state[page_key] -= 1
      str_lit.rerun()
  with p_col3:
  ...
  with p_col4:
    if str_lit.button(
        "▶️",
        key=f"{page_key}_next_{key_suffix}",
  ...
  with p_col5:
    if str_lit.button(
        "⏭️",
        key=f"{page_key}_last_{key_suffix}",
        disabled=current_page == total_pages,
    ):
...
@str_lit.dialog("Manage Card Inventory")
def manage_card_inventory_dialog(group, user_id, all_existing_tags):
  ...
  str_lit.markdown("#### **Current Copies & Tags**")
  ...
  for entry in entries:
    ...
    str_lit.markdown(f"**{finish}** ({cond})")

    new_qty = str_lit.number_input(
        "Qty", min_value=0, value=int(qty), key=f"dlg_qty_{entry_id}"
    )
    ...
    if str_lit.button("💾 Save Changes", key=f"dlg_save_{entry_id}"):
      if new_qty == 0:
        ...
        str_lit.toast("Variant deleted", icon="🗑️")
      else:
        parsed_new = [
            t.strip() for t in new_custom_tags.split(",") if t.strip()
        ]
        ...
        str_lit.toast("Updated variant & tags", icon="✅")
      str_lit.rerun(scope="app")

    str_lit.divider()

  str_lit.markdown("#### **Add Variant**")
  with str_lit.form(key=f"add_variant_form_{scryfall_id}"):
    fc1, fc2, fc3 = str_lit.columns(3)
    with fc1:
      new_finish = str_lit.selectbox(
          "Finish",
          options=["nonfoil", "foil", "etched"],
          key=f"add_fin_{scryfall_id}",
      )
    with fc2:
      new_cond = str_lit.selectbox(
          "Condition",
          options=[
              "Near Mint",
              "Lightly Played",
    ...
    with fc3:
      add_qty = str_lit.number_input(
          "Quantity", min_value=1, value=1, key=f"add_qty_{scryfall_id}"
      )

    if str_lit.form_submit_button("➕ Add Entry", width="stretch"):
      card_data = fetch_cached_card(scryfall_id)
      price_val = 0.0
      if card_data:
        p_str = card_data.get("prices", {}).get(
            "usd_foil" if new_finish == "foil" else "usd"
        )
        try:
      ...
      str_lit.toast("Variant added successfully!", icon="✅")
      str_lit.rerun(scope="app")
...
if str_lit.session_state.user is None:
  ...
  with col2:
    ...
    with str_lit.form("login_form", clear_on_submit=False):
      ...
      login_password = str_lit.text_input(
...
# --- AUTHENTICATED VIEW (APP DASHBOARD) ---
  ...
  # Build lookup map for user library quantities to display on Search tab
  library_qty_map = {}
  for entry in all_lib_raw:
  ...
  with str_lit.container():
    str_lit.markdown(
        '<div class="sticky-header-marker"></div>', unsafe_allow_html=True
    )
    col_brand, col_nav, col_user = str_lit.columns(
        [1.2, 3.8, 1.2], vertical_alignment="center"
    )
    ...
    if current_tab == "🔍 Search":
      search_col, sort_col = str_lit.columns(
          [3.5, 1.5], vertical_alignment="center"
      )
      with search_col:
          ...
          for i, tag in enumerate(sorted_tags):
            ...
            with tag_cols[col_idx]:
              if str_lit.checkbox(
                  tag,
                  key=f"tag_cb_{tag}",
                  on_change=lambda: str_lit.session_state.update(
                      {"lib_page": 1}
                  ),
              ):
  ...
  def render_search_row(card, idx, library_qty_map):
    c_preview, c_info, c_price, c_actions = str_lit.columns(
        [1.8, 3.0, 1.5, 2.2]
    )
    img_url = get_card_image_url(card, size="large") or get_card_image_url(
        card, size="normal"
    )
    ...
    with c_actions:
      ...
      if b1.button("➕ Reg", key=f"add_reg_{card_id}_{idx}"):
        ...
        str_lit.toast(f"Added {c_name} (Reg)", icon="✅")
      ...
      if b2.button("✨ Foil", key=f"add_foil_{card_id}_{idx}"):
        ...
        str_lit.toast(f"Added {c_name} (Foil)", icon="✨")
        str_lit.rerun(scope="fragment")
      ...
      if b3.button("❤️", key=f"wish_{card_id}_{idx}"):
        ...
        str_lit.toast("Added to Wishlist", icon="❤️")
        str_lit.rerun(scope="fragment")
  ...
  def render_library_row(group, sorted_tags):
    c_preview, c_info, c_details, c_price_qty, c_action = str_lit.columns(
        [0.5, 2.5, 2.0, 1.5, 1.5], vertical_alignment="center"
    )
    ...
    if unit_price is None or not card_name or not img_url:
      ...
      if card_data:
        ...
        img_url = get_card_image_url(card_data, size="large") or get_card_image_url(
            card_data, size="normal"
        )
    ...
    with c_details:
      variant_str = ", ".join([
          (
              f"{e.get('quantity', 1)}x"
              f" {e.get('finish', 'nonfoil').capitalize()}"
              f" ({e.get('condition', 'NM')})"
          )
          for e in entries
      ])
      str_lit.caption(variant_str)
      if all_tags:
    ...
    with c_price_qty:
      ...
      str_lit.markdown(
          f"<p style='text-align: right; margin: 0;'>Price:"
          f" <b>{price_display}</b></p>",
          unsafe_allow_html=True,
      )
      str_lit.markdown(
          f"<h3 style='text-align: right; margin: 0;'>{total_qty}x Total</h3>",
          unsafe_allow_html=True,
      )

    with c_action:
      if str_lit.button(
          "⚙️ Manage", key=f"btn_manage_{scryfall_id}", width="stretch"
      ):
  ...
  def render_wishlist_row(wish_item, idx):
    c_preview, c_info, c_price, c_actions = str_lit.columns(
        [.4, 3.0, 1.5, 2.2], vertical_alignment="center"
    )
    ...
    if (
        unit_price is None
        or not wish_item.get("card_name")
        or not wish_item.get("image_url")
    ):
      if card_data:
    ...
    tcg_url = (
        card_data.get("purchase_uris", {}).get("tcgplayer")
        if card_data
        else None
    )
    ...
    with c_price:
      price_display = f"${unit_price:.2f}" if unit_price is not None else "N/A"
      str_lit.markdown(f"Price: **{price_display}**")
    ...
    with c_actions:
      ...
      with b_rem:
        if str_lit.button(
            "❌ Remove", key=f"rem_w_{scryfall_id}_{idx}", width="stretch"
        ):
    ...
    if current_tab == "🔍 Search":
      if search_query:
        ...
        if sort_option == "Name (A-Z)":
          ...
          results = sorted(
              results, key=lambda c: c.get("name", "").lower(), reverse=True
          )
          ...
          results = sorted(
              results, key=lambda c: c.get("released_at", ""), reverse=True
          )
        elif sort_option == "Released: Oldest":
          results = sorted(results, key=lambda c: c.get("released_at", ""))

        str_lit.caption(
            f"Found **{len(results)}** printings for `{search_query}` (Sorted"
            f" by: {sort_option})"
        )
      ...
      total_lib_pages = max(
          1, (total_lib_items + lib_page_size - 1) // lib_page_size
      )
      ...
      total_wish_pages = max(
          1, (total_wish_items + wish_page_size - 1) // wish_page_size
      )
